...
export function runtimeState(repoRoot: string): RuntimeState {
  ...
  try { head = git(repoRoot, ['rev-parse', 'HEAD']); } catch { /* not a repo */ }
  try { branch = git(repoRoot, ['rev-parse', '--abbrev-ref', 'HEAD']); } catch { /* detached */ }
  try { dirty = git(repoRoot, ['status', '--porcelain']).length > 0; } catch { /* ignore */ }
